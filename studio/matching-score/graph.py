...
SKILL_EQUIVALENTS: dict[str, set[str]] = {
    ...
    "backend programming fundamentals": {"OOP", "Golang", "Go", "Node.js", "Java", "Express.js"},

    "debugging and troubleshooting": {"debugging", "troubleshooting", "Problem Solving"},
...
def skill_requirement_detail(
    ...
    if len(alternatives) > 1:
        ...
        if any(detail["status"] == "matched" for detail in alternative_details):
            ...
            matched = next(
                ...
                detail for detail in alternative_details if detail["status"] == "matched"
        ...
        if any(detail["status"] in {"unknown", "ambiguous"} for detail in alternative_details):
    ...
    if composite:
        ...
        term_details = [skill_requirement_detail(term, canonical_user_skills) for term in terms]

        matched_terms = [

            term for term, detail in zip(terms, term_details) if detail["status"] == "matched"
...
def match_skills(requirements: list[str], user_skills: set[str]) -> dict[str, Any]:
    ...
    if not cleaned_requirements:
        ...
        return {
            ...
            **make_match_detail("not_specified", None, reason="no skill requirements provided"),
    ...
    details = [
        ...
        {"requirement": requirement, **skill_requirement_detail(requirement, user_skills)}
    ...
    unknown = [item["requirement"] for item in details if item["status"] in {"unknown", "ambiguous"}]
...
def role_match_detail(cv_roles: list[str], job_roles: list[str]) -> dict[str, Any]:
    ...
    if not job_roles:
        ...
        return make_match_detail("not_specified", None, reason="no role requirements provided")
    ...
    for role in job_roles:
        ...
        matched_role = next((candidate for candidate in cv_norm if job_key in candidate or candidate in job_key), None)
        ...
        if matched_role:
            ...
            evidence.append(
                ...
                {"requirement": role, "matched_value": matched_role, "matched_by": "role_text"}

            )

    ratio = hits / len(job_roles)

    status: MatchStatus = "matched" if hits == len(job_roles) else "partial" if hits else "mismatched"
...
def seniority_match_detail(
    ...
    if is_unknown(job_seniority):
        ...
        return make_match_detail("not_specified", None, reason="job seniority is not specified")

    if is_unknown(cv_seniority):

        return make_match_detail("unknown", None, reason="candidate seniority is missing")
    ...
    if cv not in SENIORITY_ORDER or job not in SENIORITY_ORDER:
        ...
        return make_match_detail("ambiguous", None, reason="unrecognized seniority value")

    gap = SENIORITY_ORDER.index(job) - SENIORITY_ORDER.index(cv)

    ratio = 1.0 if gap <= 0 else 0.6 if gap == 1 else 0.3 if gap == 2 else 0.0

    status: MatchStatus = "matched" if ratio == 1.0 else "partial" if ratio else "mismatched"
    ...
    return make_match_detail(
        ...
        evidence=[{"candidate": cv_seniority, "requirement": job_seniority, "matched_by": "seniority_order"}],
...
def experience_match_detail(
    ...
    if job_min is None and job_max is None:
        ...
        return make_match_detail("not_specified", None, reason="job experience range is not specified")

    if cv_years is None:

        return make_match_detail("unknown", None, reason="candidate experience is missing")
    ...
    try:
        ...
        return make_match_detail("ambiguous", None, reason="experience values are not numeric")
    ...
    return make_match_detail(
        ...
        evidence=[{"candidate": cv_years, "minimum": job_min, "maximum": job_max, "matched_by": "experience_range"}],
...
def enum_match_detail(
    ...
    if is_unknown(job_value):
        ...
        return make_match_detail("not_specified", None, reason="job enum is not specified")
    ...
    if not values:
        ...
        return make_match_detail("unknown", None, reason="candidate enum preference is missing")
    ...
    for value in values:
        ...
        if job_key == cv_key:
            ...
            return make_match_detail(
                ...
                evidence=[{"candidate": value, "requirement": job_value, "matched_by": "enum_exact"}],
...
def salary_match_detail(cv_features: dict[str, Any], job_features: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        return make_match_detail("not_specified", None, reason="job salary is not specified")
    ...
    if cv_min is None and cv_max is None:
        ...
        return make_match_detail("unknown", None, reason="candidate salary expectation is missing")
    ...
    if job_currency and expected.get("currency"):
        ...
        if str(job_currency).casefold() != str(expected["currency"]).casefold():
            ...
            return make_match_detail("mismatched", 0.0, reason="salary currencies differ")
    ...
    if cv_ask is None:
        ...
        return make_match_detail("ambiguous", None, reason="candidate salary is incomplete")

    if job_max is not None and cv_ask > job_max:

        return make_match_detail("mismatched", 0.0, reason="candidate salary exceeds job maximum")

    if job_min is not None and cv_max is not None and cv_max < job_min:

        return make_match_detail("mismatched", 0.0, reason="candidate salary is below job minimum")
    ...
    return make_match_detail(
        ...
        evidence=[{"candidate_minimum": cv_min, "candidate_maximum": cv_max, "job_minimum": job_min, "job_maximum": job_max, "matched_by": "salary_range"}],
...
def score_match(cv_result: dict[str, Any], job_result: dict[str, Any]) -> dict[str, Any]:

    for label, extraction in (("CV", cv_result), ("job", job_result)):

        if extraction.get("validation_status") not in (None, "valid"):

            raise ValueError(f"{label} extraction is invalid: {extraction.get('validation_errors') or extraction.get('warnings')}")
    ...
    dimension_details: dict[str, dict[str, Any]] = {
        ...
        "role_match": role_match_detail(cv.get("role_tags") or [], job.get("role_tags") or []),
        ...
        "work_type": enum_match_detail(cv.get("preferred_work_types") or [], job.get("work_type")),
    ...
    warnings = list((cv_result.get("warnings") or []) + (job_result.get("warnings") or []))
    ...
    review_reasons.extend(f"candidate ambiguous field: {field}" for field in candidate_ambiguous)
    ...
    review_reasons.extend(f"confirmation required: {field}" for field in confirmation_required)
